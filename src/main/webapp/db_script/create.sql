...
CREATE TABLE `i_cat` (
  ...
  `orderr` tinyint(4) NULL COMMENT '顺序',
...
CREATE TABLE `i_com` (
  ...
  `sales` INT(11) NULL COMMENT '总销量',
  `day_sales` INT(11) NULL COMMENT '昨日销量',
  `week_salse` INT(11) NULL COMMENT '一周内销量',
  `state` varchar(12) DEFAULT NULL COMMENT '状态（ONLINE：1, OFFLINE：0 ）',
  `pric_cat_id` bigint(20) unsigned NOT NULL COMMENT '价格类目id',
  `pack_cat_id` bigint(20) unsigned NOT NULL COMMENT '包装类型id',
  `pack_cat` VARCHAR(100) NULL COMMENT '包装类型名称',
  `biz_id` bigint(20) unsigned NOT NULL COMMENT '商户id',
...
CREATE TABLE `i_order` (
  ...
  `exp_del_time_des` VARCHAR(300) NULL COMMENT 'expected_delivery_time_des预计送达时间描述',
  `time` VARCHAR(20) NULL COMMENT '订单完成用时',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `modify_time` datetime NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `i_user` (
  ...
  `freezer_type` varchar(200) NULL COMMENT '冰柜种类',
  `district_id` bigint(200) NULL COMMENT '所属片区ID',
  `standby_phone` varchar(18) NULL COMMENT '备用手机号',
